  ...
  <div>
    <div v-if="selection.length > 0" class="clipboard-container">
      ...
      >
        ...
        <v-btn v-if="canAddAlbums" key="action-album" :title="$gettext('Add to album')" icon="mdi-bookmark" color="album" density="comfortable"  :disabled="selection.length === 0" class="action-album" @click.stop="dialog.album = true"></v-btn>
