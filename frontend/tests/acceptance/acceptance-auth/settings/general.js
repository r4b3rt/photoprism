...
test.meta("testID", "settings-general-002").meta({ type: "short", mode: "auth" })("Common: Change language", async (t) => {
  ...
  await menu.openPage("settings");
  await t.click(settings.languageOpenSelection).hover(Selector("div").withText("Deutsch").parent('div[role="option"]')).click(Selector("div").withText("Deutsch").parent('div[role="option"]'));
  await t.eval(() => location.reload());

  await t.expect(Selector("a.nav-browse").innerText).contains("Suche");

  await t.click(settings.languageOpenSelection).hover(Selector("div").withText("English").parent('div[role="option"]')).click(Selector("div").withText("English").parent('div[role="option"]'));

  await t.expect(Selector("a.nav-browse").innerText).contains("Search");
});

test.meta("testID", "settings-general-003").meta({ type: "short", mode: "auth" })("Common: Disable pages: import, originals, logs, moments, places, library", async (t) => {
  ...
  await menu.openPage("settings");
  await t.click(settings.importCheckbox).click(settings.filesCheckbox).click(settings.momentsCheckbox).click(settings.logsCheckbox).click(settings.placesCheckbox);
  await t.eval(() => location.reload());
  ...
  await menu.openPage("settings");
  await t.click(settings.importCheckbox).click(settings.filesCheckbox).click(settings.momentsCheckbox).click(settings.logsCheckbox).click(settings.placesCheckbox).click(settings.libraryCheckbox);
...
test.meta("testID", "settings-general-006").meta({ type: "short", mode: "auth" })("Common: Disable upload, download, edit and share", async (t) => {
  ...
  await t.expect(photoedit.downloadFile.nth(0).visible).ok().click(photoedit.toggleExpandFile.nth(1)).expect(photoedit.downloadFile.nth(0).visible).ok().expect(photoedit.deleteFile.visible).ok().click(photoedit.dialogClose);
